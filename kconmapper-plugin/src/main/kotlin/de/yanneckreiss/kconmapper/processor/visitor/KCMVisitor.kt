...
private const val KCONMAPPER_CLASSES_ANNOTATION_ARG_NAME = "classes"
...
class KCMVisitor(
    ...
    private fun extractKCMAnnotation(targetClass: KSClassDeclaration): KSAnnotation {
        ...
        kcmAnnotation.arguments.firstOrNull { constructorParam ->
            constructorParam.name?.asString() == KCONMAPPER_CLASSES_ANNOTATION_ARG_NAME
        } ?: run {
            logger.logAndThrowError(
    ...
    private fun extractOriginClasses(kcmAnnotation: KSAnnotation): List<KSType> {
        return kcmAnnotation
            .arguments
            .find { annotationArgument: KSValueArgument -> annotationArgument.name?.asString() == KCONMAPPER_CLASSES_ANNOTATION_ARG_NAME }!!
